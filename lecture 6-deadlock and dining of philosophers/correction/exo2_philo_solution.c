...
#define NUM_DINERS 3
#define EAT_TIMES 20
#define NUM_EATING (NUM_DINERS - 1)
// Le nombre maximal de philosophes qui peuvent manger en même temps est égal au
// nombre de philosophes moins 1.
// Il suffit que l'un d'eux, n'utilise pas sa fourchette pour qu'un autre puisse
// l'utiliser : quand il aura mangé et reposé ses fourchettes, il permettra à un
// autre de manger à son tour et ainsi de suite...
...
void* philo_thread(void* arg) {
    ...
    for(i = 0; i < EAT_TIMES; i++) {
        sem_wait(p.numEating); // on limite le nombre de philosophe qui mange
        ...
        sem_post(p.numEating); // on limite le nombre de philosophe qui mange

...
//déclaration d'une constante à utiliser dans la boucle des tâches
...
printf("Task %s reads the shared data\n", (char*) arg);
//reste du code
i++;
}
}
...
int i = 0;
while(i < N){
usleep(5000); //attendre 5 secondes
printf("Tache %s is executing\n", (char*) arg);
...
pthread_mutex_unlock(&verrou);
printf("Task %s writes on shared data\n", (char*) arg);
//reste du code
}
}
...
pthread_mutexattr_setprioceiling(&m_attr, 15); /*le mutex dispose d'une priorité de 15, qui sera celle de la tâche*/ pthread_mutexattr_setprotocol(&m_attr, PTHREAD_PRIO_PROTECT);
